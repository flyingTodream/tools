...
function randomizeObject(object, parse = null) {
  ...
  if (object instanceof Array) {
    ...
    object.createdRepeatedArray instanceof Array
  ) {
...
function createRepeatedArray(inputArray) {
  ...
  if (
    ...
    let repeatNum = parseInt(inputArray[0].match(/repeat\[(\d*)\]/)[1])
    while (repeatNum > 0) {
      ArrayText.push(
...
function getValuesArray(command) {
  ...
  if (command == 'bool') {
    ...
    return command.match(/int\(.*?\)/)
      ? getIntArray(command)
      : getStringArray(command)
  } else if (command == 'int') {
    //No i18N
...
function getIntArray(command) {
  ...
  command = command.replace(/(\d+)\-(\d+)/g, function (match, p1, p2) {
    let elementsString = ''
    let lessItem = parseInt(p1) < parseInt(p2) ? parseInt(p1) : parseInt(p2)
    const moreItem = parseInt(p1) < parseInt(p2) ? parseInt(p2) : parseInt(p1)
