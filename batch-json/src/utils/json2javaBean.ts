...
function camelCase(input: string) {
  return input.replace(/_(.)/g, function (match: any, group1: string) {
...
function isDate(date: any) {
  return (
    new Date(date) !== 'Invalid Date' && !isNaN(new Date(date)) && isNaN(+date)
  )
}
