<template>
  <h-single-layout mode="middle" :width="1280">
    <div class="encrypt">
      ...
      <h-multiline-input
          v-model="publicKey"
          :title="t('encrypt.title.public')"
      />
      <p class="encrypt-button">
        <a-button type="primary" @click="handleEncrypt">{{
            t('encrypt.button')
          }}
        </a-button>
      </p>
      <h-multiline
          :result-desc="t('encrypt.title.encrypted')"
...
let EXAMPLE_MESSAGE = 'A Free, Modern Toolbox, Built for Developers.'
