<template>
  <h-single-layout :max-width="1280">
    <div class="input">
      <!-- <span>请在此处输入内容</span> -->
      <h-multiline-input v-model:modelValue="inputValue" autofocus class="input" :title="t('input_tip')"
        @change="decode" />
    </div>
    ...
    <span>{{ t('result_tip_no_colon') }}</span>
    <a-textarea v-model:value="outputValue" :autofocus="false" @change="encode" />
  </h-single-layout>
</template>
