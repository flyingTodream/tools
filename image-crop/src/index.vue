      ...
      >
        <a-form-item style="justify-content: center">
          <h-file-upload-button
            accept="image/*"
            size="middle"
            @after-selected="handleFileSelected"
          >
            {{ t('choose') }}
          </h-file-upload-button>
        </a-form-item>
        ...
        <a-form-item :label="t('rotate')">
          <a-input-number
            v-model:value="imageMessage.rotate"
            max="360"
            min="-360"
          />
        </a-form-item>
...
onMounted(() => {
  ...
  imageRef.value?.addEventListener('cropend', () => {
    ...
    imageMessage.height = data.height >> 0;
    imageMessage.width = data.width >> 0;
    imageMessage.x = data.x >> 0;
    imageMessage.y = data.y >> 0;
  });
  // 图片加载完毕
