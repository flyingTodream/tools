...
export async function generateRSAKeyPair(options: Option): Promise<RSAKeyPair> {
  ...
  switch (options.privateKeyFormat) {
    case 'OpenSSH':
      res.privateKey = forge.ssh.privateKeyToOpenSSH(
        kp.privateKey,
        options.passphrase,
      )
      break
    ...
    case 'Pem':
      if (options.passphrase !== '') {
        res.privateKey = forge.pki.encryptRsaPrivateKey(
          kp.privateKey,
          options.passphrase,
        )
      } else {
        res.privateKey = forge.pki.privateKeyToPem(
          kp.privateKey,
          options.pemCharPerLine,
        )
      }
      break
  }
  ...
  switch (options.publicKeyFormat) {
    case 'OpenSSH':
      res.publicKey = forge.ssh.publicKeyToOpenSSH(
        kp.publicKey,
        options.publicKeyComment,
      )
      break
    case 'Pem':
      res.publicKey = forge.pki.publicKeyToRSAPublicKeyPem(
        kp.publicKey,
        options.pemCharPerLine,
      )
      break
...
export async function getRSAPublicKeyFromPrivateKey(
  ...
  try {
    ...
    return Promise.reject(
      'Failed to decrypt the private key. Please check your passphrase.',
    )
  }
  return res
}
